...
fun AnimatedFrames(modifier: Modifier, feel: FalehFeel) {
    ...
    val sleepFrames = remember {
        listOf(
            Res.drawable.sleep_frame1,
            Res.drawable.sleep_frame2,
    ...
    val painters = when (feel) {
        FalehFeel.IDLE -> idleFrames.map { painterResource(it) }
        FalehFeel.SLEEP -> sleepFrames.map { painterResource(it) }
        FalehFeel.CORRECT -> correctAnswersFrames.map { painterResource(it) }
    }

    val duration = when (feel) {
        FalehFeel.IDLE -> IDLE_TIME
        FalehFeel.SLEEP -> SLEEP_TIME
        FalehFeel.CORRECT -> CORRECT_TIME
    }
...
private fun FeelsAnimationWidget(
    ...
    val lastIndex = painters.lastIndex

    key(painters.size) {
        val rawIndex by rememberInfiniteTransition(label = "feelTransition").animateValue(
            initialValue = 0,
        ...
        val safeIndex = rawIndex.coerceIn(0, lastIndex)



        Image(
            painter = painters[safeIndex],
